...
def search_gene_sequences(
    ...
    """
    Search NCBI for sequences of a specific gene in the target organism.
    Used for gene-targeted mode (e.g. cpn60 for Gardnerella, rpoB for Mycobacterium).
    Returns a list of SeqRecord objects.
    """
    _configure_entrez(email, api_key)
    ...
    for q in queries:
        logger.info("Searching NCBI for gene '%s' in '%s': %s", gene, organism, q)
        try:
            handle = Entrez.esearch(db="nucleotide", term=q, retmax=max_seqs, usehistory="y")
            record = Entrez.read(handle)
            handle.close()
            ids = record.get("IdList", [])
            if ids:
                logger.info("Found %d sequence(s) for gene '%s' in '%s'", len(ids), gene, organism)
                recs = fetch_sequences(ids, email=email, api_key=api_key)
                if recs:
                    return recs
        except Exception as exc:
            logger.warning("Gene search failed (%s): %s", q, exc)
            time.sleep(1)

    logger.warning("No gene sequences found for '%s' in '%s'", gene, organism)
    return []


def get_close_relative_gene_sequences(
    organism: str,
    gene: str,
    n: int = 10,
    email: str = "",
    ...
    """
    Fetch gene sequences from close relatives for exclusion in gene-targeted mode.
    """
    ...
    queries = [
        f'"{genus}"[Organism] NOT "{species_query}"[Organism] AND "{gene}"[Gene] AND refseq[filter]',
        f'"{genus}"[Organism] NOT "{species_query}"[Organism] AND "{gene}"[Title]',
        f'"{genus}"[Organism] NOT "{species_query}"[Organism] AND "{gene}"[All Fields]',
    ]

    _configure_entrez(email, api_key)
    for q in queries:
        logger.info("Searching relative gene sequences: %s", q)
        try:
            handle = Entrez.esearch(db="nucleotide", term=q, retmax=n)
            record = Entrez.read(handle)
            handle.close()
            ids = record.get("IdList", [])
            if ids:
            ...
            logger.warning("Relative gene search failed: %s", exc)
            time.sleep(1)
